import torch
import torch.nn as nn
from PIL import Image, ImageDraw
import numpy as np
import tkinter as tk
from tkinter import messagebox
import cv2
import torchvision.transforms as transforms


# ==========================
#       MODEL DEFINITION
# ==========================
class ImprovedCNN(nn.Module):
    def __init__(self, num_classes):
        super(ImprovedCNN, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(32, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Dropout(0.25)
        )
        self.layer2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Dropout(0.25)
        )
        self.fc = nn.Sequential(
            nn.Linear(64 * 7 * 7, 256),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(256, num_classes)
        )

    def forward(self, x):
        x = self.layer1(x)
        x = self.layer2(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x


# ==========================
#            LABELS
# ==========================
LABELS = {
   1: "а", 2: "б", 3: "в", 4: "г", 5: "д", 6: "е", 7: "ё", 8: "ж",
   9: "з", 10: "и", 11: "й", 12: "к", 13: "л", 14: "м", 15: "н", 16: "о",
   17: "ө", 18: "п", 19: "р", 20: "с", 21: "т", 22: "у", 23: "ү", 24: "ф",
   25: "х", 26: "ц", 27: "ч", 28: "ш", 29: "щ", 30: "ъ", 31: "ы", 32: "ь",
   33: "э", 34: "ю", 35: "я"
}


# ==========================
#      LOAD MODEL
# ==========================
model = ImprovedCNN(35)
model.load_state_dict(torch.load("model_cnn.pth", map_location="cpu"))
model.eval()


# ==========================
#    PREPROCESS CHARACTER
# ==========================
def preprocess_single_char(roi_binary):
    """
    roi_binary: numpy array (H, W) with 0 background and 255 foreground (white on black)
    returns: tensor shape (1, 1, 28, 28)
    """
    # Convert to PIL grayscale
    img = Image.fromarray(roi_binary).convert("L")

    # bounding box crop (safety)
    bbox = img.getbbox()
    if bbox:
        img = img.crop(bbox)

    # maintain aspect ratio: resize longest side to 20 and pad to 28x28 (like MNIST)
    w, h = img.size
    if w > h:
        new_w = 20
        new_h = int(round((20.0 * h) / w))
        if new_h == 0:
            new_h = 1
    else:
        new_h = 20
        new_w = int(round((20.0 * w) / h))
        if new_w == 0:
            new_w = 1

    img = img.resize((new_w, new_h), Image.LANCZOS)

    # pad to 28x28 centered
    padded = Image.new("L", (28, 28), 0)
    paste_x = (28 - new_w) // 2
    paste_y = (28 - new_h) // 2
    padded.paste(img, (paste_x, paste_y))

    transform = transforms.Compose([
        transforms.ToTensor(),  # will produce [0,1], shape (1,28,28)
    ])
    return transform(padded).unsqueeze(0)


# ==========================
#         DRAW APP
# ==========================
class DrawApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Mongolian Multi-Character Recognition (Connected Components)")

        info = tk.Label(root, text="Draw characters (WHITE on BLACK). Separate them by SPACE.",
                        font=("Arial", 11), fg="yellow")
        info.pack(pady=5)

        self.canvas = tk.Canvas(root, width=1200, height=400, bg='black', cursor="cross")
        self.canvas.pack(pady=10)

        # PIL image to record strokes
        self.image = Image.new("L", (1200, 400), 0)
        self.draw = ImageDraw.Draw(self.image)

        # Track previous mouse position
        self.last_x = None
        self.last_y = None

        self.canvas.bind("<B1-Motion>", self.paint)
        self.canvas.bind("<ButtonRelease-1>", self.reset)

        btn_frame = tk.Frame(root)
        btn_frame.pack()

        tk.Button(btn_frame, text="Recognize All", command=self.recognize,
                  bg='green', fg='white', width=20).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Clear", command=self.clear,
                  bg='red', fg='white', width=20).pack(side=tk.LEFT, padx=5)

        self.result_label = tk.Label(root, text="", font=("Arial", 20, "bold"))
        self.result_label.pack(pady=10)

        self.confidence_label = tk.Label(root, text="", font=("Arial", 11))
        self.confidence_label.pack(pady=5)

    # ===== SMOOTH PAINT =====
    def paint(self, event):
        if self.last_x is not None and self.last_y is not None:
            # Tkinter draw
            self.canvas.create_line(
                self.last_x, self.last_y, event.x, event.y,
                fill="white", width=22, capstyle="round", smooth=True
            )

            # PIL draw
            self.draw.line(
                [self.last_x, self.last_y, event.x, event.y],
                fill=255, width=22
            )

        self.last_x, self.last_y = event.x, event.y

    def reset(self, event):
        self.last_x, self.last_y = None, None

    def clear(self):
        self.canvas.delete("all")
        self.image = Image.new("L", (1200, 400), 0)
        self.draw = ImageDraw.Draw(self.image)
        self.result_label.config(text="")
        self.confidence_label.config(text="")

    # ===== RECOGNITION =====
    def recognize(self):
        img_array = np.array(self.image)

        # threshold (ensure binary 0/255)
        _, binary = cv2.threshold(img_array, 127, 255, cv2.THRESH_BINARY)

        # === CONNECTED COMPONENTS (No reordering: process in label order) ===
        # returns: num_labels, label_image, stats, centroids
        num_labels, labels_img, stats, centroids = cv2.connectedComponentsWithStats(binary, connectivity=8)

        if num_labels <= 1:
            messagebox.showinfo("Error", "No characters found!")
            return

        recognized_text = ""
        confidence_list = []

        # Process components in label order (1..num_labels-1). This preserves the
        # scan order assigned by connectedComponentsWithStats (top-to-bottom, left-to-right raster).
        for i in range(1, num_labels):
            x, y, w, h, area = stats[i]

            # Skip tiny components (noise)
            if area < 40:
                continue

            # extract ROI
            roi = binary[y:y+h, x:x+w]

            # If ROI is empty somehow, skip
            if roi.size == 0:
                continue

            # Preprocess and predict
            tensor = preprocess_single_char(roi)

            with torch.no_grad():
                output = model(tensor)
                probs = nn.Softmax(dim=1)(output)
                conf, pred = torch.max(probs, 1)

            # map prediction (0..34) to LABELS dict keys (1..35)
            label_key = pred.item() + 1
            # safety: if label_key not in labels, skip/replace
            char = LABELS.get(label_key, "?")

            recognized_text += char
            confidence_list.append(f"{char}({conf.item()*100:.0f}%)")

        if recognized_text == "":
            messagebox.showinfo("Error", "No characters recognized (all components filtered as noise).")
            return

        self.result_label.config(text=f"Result: {recognized_text}")
        self.confidence_label.config(text=" | ".join(confidence_list))


# ==========================
#          RUN APP
# ==========================
if __name__ == "__main__":
    root = tk.Tk()
    app = DrawApp(root)
    root.mainloop()
